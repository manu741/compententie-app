        ...
        <div class="flex justify-center">
            <div class="bg-gray-100 dark:bg-gray-700 w-8/12 rounded-b mb-4">
                    ...
                    <div class="relative inline-flex">
                        ...
                        <select name="competentie" id="competentie_drop" class="border border-gray-300 text-gray-600 bg-white hover:border-gray-400 focus:outline-none appearance-none px-3 py-2 dark:bg-gray-600 dark:text-gray-400 dark:border-gray-500 dark:hover:border-gray-800 hover:border-gray-400" style="width:180px;">
                            ...
                            @foreach($indicatoren as $indicator)
                                <option value="{{ $indicator->id }}">{{$indicator->competentie}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="mt-2 mb-2 ">
                    ...
                    <div class="relative inline-flex">
                        <svg class="w-2 h-2 absolute top-0 right-0 m-4 pointer-events-none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 412 232"><path d="M206 171.144L42.678 7.822c-9.763-9.763-25.592-9.763-35.355 0-9.763 9.764-9.763 25.592 0 35.355l181 181c4.88 4.882 11.279 7.323 17.677 7.323s12.796-2.441 17.678-7.322l181-181c9.763-9.764 9.763-25.592 0-35.355-9.763-9.763-25.592-9.763-35.355 0L206 171.144z" fill="#648299" fill-rule="nonzero"/></svg>
                        <span id="indicator_omschrijving"></span>
    ...
    <script>
        $(document).ready(function () {
            $('#competentie_drop').on('change', function () {
                let id = $(this).val();
                $('#niveau_drop').empty();
                $('#niveau_drop').append(`<option value="0" disabled selected>Kies een optie...</option>`);
                $.ajax({
                    type: 'GET',
                    ...
                    success: function (response) {
                        ...
                        response.forEach(element => {
                            $('#niveau_drop').append(`<option value="${element['id']}">${element['niveau']}</option>`);
                        });
                        $('#niveau_drop').on('change', function () {
                            console.log(response[0]['indicator']); //HELPP!
                            // $("select[name='niveau'] option:selected").index() - 1
                        });
                    }
                })
            })
